...
- [Cloud™ Examples](#cloud-examples)
  ...
  - [☁ DigitalOcean](#%E2%98%81-digitalocean)
...
- ☁️ Deploy a single container image or `docker-compose.yaml` manifest to a VM running on any of the following clouds:
...
- 🔑 Automatic SSL/TLS certificates generated by **Let's Encrypt** (courtesy of Traefik 👆).
- 🛠 Extremely customisable using environment variables or your own cloud-init config
...
### 1️⃣ Deploying a single container
...
module "cloudinit" {
  ...
  domain = "example.com"
  image  = "nginx:latest" # 🐳 deploy a single container
...
### 2️⃣ Deploying a `docker-compose.yaml` file
...
module "cloudinit" {
  ...
  domain         = "example.com"
  docker_compose = file("docker-compose.yaml") # 🐳 deploy multiple services
...
resource "aws_instance" "vm" {
  ...
  user_data = module.cloudinit.cloud_config # 👈
}
...
resource "google_compute_instance" "vm" {
  ...
  metadata = {
    user-data = module.cloudinit.cloud_config # 👈
  }
...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  custom_data = base64encode(module.cloudinit.cloud_config) # 👈
...
⚠️ `TODO: Describe customising Traefik through environment variables.`

> _Some info on Traefik's default config:_
>
> - 🔗 Traefik connects to services over the Docker network `traefik` — any service(s) you want routed through Traefik need to be on this network.
> - 🔒 Let's Encrypt is enabled via the `letsencrypt` certificate resolver and by default uses the [HTTP challenge](https://doc.traefik.io/traefik/user-guides/docker-compose/acme-http/) method for validating domains.
> - 📋 Traefik's static configuration is set through environment variables. You can override any value by providing your own environment variable definition via the `environment` input variable.
> - 📊 By default the module enables Traefik's [dashboard](https://docs.traefik.io/operations/dashboard/) and API. Both are accessible from the `traefik` subdomain (e.g. `https://traefik.${domain}/dashboard/`) using the username/password: `admin`/`traefik`.

### Cloud-Init

⚠️ `TODO: Describe extending module through additional cloud-init steps.`
